...
export async function createGame(hostName: string): Promise<{ game: GameState; playerId: string } | null> {
  try {
...
export async function dealCards(gameId: string, estateKeeperId: string): Promise<boolean> {
  try {
    ...
    for (const player of playersData) {
      const identity = drawRandomCard('identity', usedCards.identity);
      usedCards.identity.push(identity);

      const relationship = drawRandomCard('relationship', usedCards.relationship);
      usedCards.relationship.push(relationship);

      const backstory1 = drawRandomCard('backstory', usedCards.backstory);
      usedCards.backstory.push(backstory1);

      const backstory2 = drawRandomCard('backstory', usedCards.backstory);
...
export async function drawCardForPlayer(gameId: string, estateKeeperId: string, playerId: string, cardType: CardType): Promise<string | null> {
  try {
    ...
    const newCard = drawRandomCard(cardType, usedCards);
